    ...
    for($row = $sheet->{MinRow}; defined $sheet->{MaxRow} && $row <= $sheet->{MaxRow}; $row++) {
        ...
        for($col = $sheet->{MinCol}; defined $sheet->{MaxCol} && $col <= $sheet->{MaxCol}; $col++) {
            ...
            if ($cell) {
                ...
                if ($cell->{Val} =~ /^\s?(Making Work Pay Tax Credit)\s?/) {
                    $rowName = getRowName("mwptc");
                    $inside = 1;
                }
                ...
                if ($inside) {
                    for ($mycol = 1; $mycol <= 70; $mycol++) {
                        ...
                        $sql = "select * from ssc_standard where locale='$tbl' and sscyear='$tblYear' and famtype='$mycol'";
                        ...
                        if ($nr == 1) {
                            $sql = "update ssc_standard set $rowName='$value' where locale='$tbl' and sscyear='$tblYear' and famtype='$mycol'\n";
                        }
                        else {
                            $sql = "insert into ssc_standard (locale,sscyear,famtype,$rowName) values ('$tbl','$tblYear','$mycol','$value')";
                            $total_number_rows++;
                        }
                        $sth = $dbh->prepare($sql) or die "Can't prepare $statement: $dbh->errstr\n";
...
sub showFinished {
    ...
    print <<"EOT";
     <div id="adminContent">
     <div class="adminHeader">SSC Standard Upload</div>

      XLS File uploaded $total_number_rows rows with $record_cnt fields processed.
          
     </div>
EOT
    include("E:/thecalculator/templates/admin/index/footer.inc");	
}
